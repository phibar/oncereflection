...
import {
  ...
  singleOrMany,
  NodeCheckFlags,
  isStatic,flatMap,nodeIsPresent,isNamedImportBindings,isEnumConst,shouldPreserveConstEnums,getOrCreateEmitNode,
  isLeftHandSideExpression,skipOuterExpressions,isAccessExpression,declarationNameToString
} from "typescript";
...
export default function transformTypeScript(context: TransformationContext) {
  const {
    factory,
    //@ts-ignore
  ...
  function serializeTypeNode(node: TypeNode | undefined): SerializedTypeNode {
    ...
    switch (node.kind) {
      ...
      case SyntaxKind.JSDocOptionalType:
        return serializeTypeNode(
          (node as JSDocNullableType | JSDocNonNullableType | JSDocOptionalType)
            .type
        );
      default:
        return Debug.failBadSyntaxKind(node);
  ...
  function transformModuleBody(
    ...
    if (node.body) {
      if (node.body.kind === SyntaxKind.ModuleBlock) {
        ...
        const moduleBlock = getInnerMostModuleDeclarationFromDottedModule(node)!
          .body as ModuleBlock;
